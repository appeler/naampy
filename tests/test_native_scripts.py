...
class TestNativeScripts(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.hindi_names_df = pd.DataFrame([{"name": "हेमा"}, {"name": "होशियार"}])
    ...
    def test_hindi_names_with_state(self):
        ...
        result = in_rolls_fn_gender(
            self.hindi_names_df, "name", state="up", dataset="v2_native"
        )
    ...
    def test_hindi_names_with_state_year(self):
        ...
        result = in_rolls_fn_gender(
            self.hindi_names_df, "name", state="up", year=1985, dataset="v2_native"
        )
    ...
    def test_v2_native_no_ml_fallback(self):
        ...
        test_names = pd.DataFrame({"name": ["हेमा", "होशियार", "unknown_native_name"]})
    ...
    def test_native_vs_transliterated_comparison(self):
        ...
        self.assertTrue(
            any(pd.notna(native_result["prop_female"])) or len(native_result) > 0
        )
        self.assertTrue(
            any(pd.notna(english_result["prop_female"])) or len(english_result) > 0
        )
            ...
            if all(
                pd.notna(
                    [
                        row.get("prop_female"),
                        row.get("prop_male"),
                        row.get("prop_third_gender"),
                    ]
                )
            ):
                # Proportions should sum to approximately 1
                prop_sum = (
                    row["prop_female"] + row["prop_male"] + row["prop_third_gender"]
                )
